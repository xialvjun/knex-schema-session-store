...
export class KnexSchemaSessionStore {
    ...
    private _synced: Promise<boolean> = null
    ...
    async get(sid: string) {
        const { table_name, sid_name, expire_at_name, additional_name, timestamps } = this._options;
        const rows = await this._knex(table_name)
            .where(sid_name, sid)
            .andWhere(expire_at_name, '>', Date.now())
            .limit(1).groupBy();
        if (rows && rows.length > 0) {
            const row = rows[0];
            let additional = row[additional_name];
            delete row[sid_name];
            delete row[expire_at_name];
            delete row[additional_name];
            if (timestamps) {
                delete row['created_at'];
                delete row['updated_at'];
            }
            return {...row, ...JSON.parse(additional)};
        }
        return null;
